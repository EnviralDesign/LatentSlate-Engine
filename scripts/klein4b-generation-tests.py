#!/usr/bin/env python3
"""Run fixed-parameter Klein 4B acceptance scenarios through the public API.

These are manual GPU tests, not pytest cases. Each scenario delegates to
``hardware-study.py`` so it exercises the same HTTP catalog, asset, job, polling,
and artifact routes used by LatentSlate while retaining one manifest per run.
"""
...
NVFP4_T2I = "flux2-klein-4b.text-to-image.bfl-distilled-nvfp4"
FP8_T2I = "flux2-klein-4b.text-to-image.comfy-distilled-fp8"
BF16_T2I = "flux2-klein-4b.text-to-image.native-distilled-bf16"
NVFP4_I2I = "flux2-klein-4b.image-to-image.bfl-distilled-nvfp4"
FP8_I2I = "flux2-klein-4b.image-to-image.comfy-distilled-fp8"
BASE_FP8_I2I = "flux2-klein-4b.image-to-image.comfy-base-fp8"
BF16_I2I = "flux2-klein-4b.image-to-image.native-distilled-bf16"
...
SCENARIOS: dict[str, tuple[RunSpec, ...]] = {
    ...
    "t2i-family": (
        RunSpec("family", (NVFP4_T2I, FP8_T2I, BF16_T2I), "t2i"),
    ),
    ...
    "i2i-family": (
        RunSpec("family", (NVFP4_I2I, FP8_I2I, BASE_FP8_I2I, BF16_I2I), "i2i"),
    ),
}
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run fixed-seed Klein 4B generation acceptance scenarios."
    )
...
def main() -> int:
    ...
    run_root = (
        ...
        or repository
        / "hardware-study-runs"
        / f"{datetime.now(UTC).strftime('%Y%m%d-%H%M%S')}-klein4b-{args.scenario}"
    ).resolve()
    run_root.mkdir(parents=True, exist_ok=False)
    summary = {
        "format": "latentslate-klein4b-acceptance-v2",
    ...
    for index, spec in enumerate(specs, start=1):
        ...
        command = [
            ...
            "--study-label",
            f"klein4b-{args.scenario}-{spec.name}",
            "--input",
            f"width={WIDTH}",
            "--input",
            f"height={HEIGHT}",
