"""Curated Qwen Image Edit 2511 transformer.

Narrowly adapted from ComfyUI 12d5279438bfefc058a269eae805ceab6047777f
ldm/qwen_image/model.py, lightricks timestep and Flux position math (GPL-3.0).
Qwen-Image upstream: QwenLM/Qwen-Image (Apache-2.0).
"""
...
def optimized_attention_masked(q, k, v, heads, mask, skip_reshape=True):
    return attention(q, k, v, mask).transpose(1, 2).reshape(q.shape[0], q.shape[2], -1)


class Timesteps(nn.Module):
    def __init__(self, num_channels, flip_sin_to_cos, downscale_freq_shift, scale):
        super().__init__()
...
class TimestepEmbedding(nn.Module):
    def __init__(self, in_channels, time_embed_dim, dtype, device, operations):
        super().__init__()
...
class QwenImageTransformer2DModel(nn.Module):
    def __init__(
        ...
        default_ref_method="index_timestep_zero",
        image_model=None,
        final_layer=True,
    ...
    def process_img(self, x, index=0, h_offset=0, w_offset=0):
        ...
        hidden_states = F.pad(x, (0, (-w) % 2, 0, (-h) % 2, 0, 0), mode="circular")
        ...
        return hidden_states, img_ids.reshape(1, -1, 3).repeat(bs, 1, 1), orig_shape

    def forward(self, x, timestep, context, ref_latents):
        hidden_states, img_ids, orig_shape = self.process_img(x)
        num_embeds = hidden_states.shape[1]
        for index, ref in enumerate(ref_latents, start=1):
            reference, ref_ids, _ = self.process_img(ref, index=index)
            hidden_states = torch.cat([hidden_states, reference], dim=1)
            img_ids = torch.cat([img_ids, ref_ids], dim=1)
        timestep = torch.cat([timestep, timestep * 0], dim=0)
        txt_start = round(max(((x.shape[-1] + 1) // 2) // 2, ((x.shape[-2] + 1) // 2) // 2))
        ...
        for block in self.transformer_blocks:
            encoder_hidden_states, hidden_states = block(
                hidden_states, encoder_hidden_states, None, temb,
                image_rotary_emb, timestep_zero_index=num_embeds,
            )
        temb = temb.chunk(2, dim=0)[0]
