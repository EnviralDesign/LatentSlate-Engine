...
class Linear(nn.Linear):
    """A linear bound to one supported Ideogram4 checkpoint representation."""
    ...
    def forward(self, x):
        ...
        if binding is None:
            raise RuntimeError("Ideogram4 linear has no loaded checkpoint")
    ...
    def __init__(self, owner, name, module, config):
        ...
        if self.format == "nvfp4":
            ...
            if (
                ...
                raise ValueError(f"Unsupported Ideogram4 INT8 ConvRot metadata: {name}")
        elif weight.dtype not in (torch.bfloat16, torch.float16, torch.float32):
            raise ValueError(f"Unsupported Ideogram4 weight representation: {name}")
        self.size = size
    ...
    def __init__(self, path: Path, model: nn.Module, device: torch.device, adapters=()):
        ...
        for name, module in model.named_modules():
            if isinstance(module, Linear):
                binding = Ideogram4Weight(self, name, module, config.get(name, {}))
        ...
        for name, parameter in list(model.named_parameters()):
            ...
            if value.shape != parameter.shape:
                raise ValueError(f"Unsupported Ideogram4 parameter shape: {name}")
            setattr(
...
class Embedding(nn.Embedding):
    def forward(self, ids, out_dtype=None):
        return F.embedding(ids, self.weight.to(out_dtype or self.weight.dtype))
...
class LayerNorm(nn.LayerNorm):
    def forward(self, x):
        return F.layer_norm(
            ...
            None if self.weight is None else self.weight.to(x.dtype),
