"""Flux2 decoder and packed latent normalization for Ideogram v4.
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.register_buffer("running_mean", torch.empty(128))
        self.register_buffer("running_var", torch.empty(128))
    ...
    def forward(self, latent):
        latent = latent * torch.sqrt(
            self.running_var.view(1, -1, 1, 1).to(latent.dtype) + 1e-4
        ) + self.running_mean.view(1, -1, 1, 1).to(latent.dtype)
        batch, _, height, width = latent.shape
        latent = (
            latent.reshape(batch, 32, 2, 2, height, width)
            .permute(0, 1, 4, 2, 5, 3)
            .reshape(batch, 32, height * 2, width * 2)
        )
...
def _decoder_key(key):
    key = key.replace("conv_shortcut", "nin_shortcut").replace(
        "conv_norm_out", "norm_out"
    )
    for i in range(4):
        key = key.replace(f"up_blocks.{i}.resnets.", f"up.{3 - i}.block.")
        key = key.replace(f"up_blocks.{i}.upsamplers.0.", f"up.{3 - i}.upsample.")
    for i in range(2):
        key = key.replace(f"mid_block.resnets.{i}.", f"mid.block_{i + 1}.")
...
def load_decoder(path: Path, device: torch.device):
    """Load only decoder parameters; text-to-image does not encode images."""
    with torch.device("meta"):
        model = Decoder()
    with safe_open(path, framework="pt") as source:
        keys = source.keys()
        state = {
            key.removeprefix("decoder."): source.get_tensor(key).to(
        ...
        for name in ("weight", "bias"):
            state["post_quant_conv." + name] = source.get_tensor(
                "post_quant_conv." + name
            ).to(device=device, dtype=torch.bfloat16)
        state = {
            _decoder_key(key): value.reshape(*value.shape, 1, 1)
            if "attentions" in key and value.ndim == 2
            else value
            for key, value in state.items()
        }
        for name in ("running_mean", "running_var"):
            state[name] = source.get_tensor("bn." + name).to(
                device=device, dtype=torch.bfloat16
            )
...
def decode(model, latent, device):
    """Decode denormalized Flux latents to CPU NHWC pixels in [0,1]."""
    pixels = model(latent.to(device=device, dtype=torch.bfloat16)).float().cpu()
