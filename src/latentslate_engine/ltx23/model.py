...
from .ops import linear_input_act, operations, optimized_attention, rms_norm
...
class FeedForward(nn.Module):
    ...
    def forward(self, x):
        ...
        if False:
            return self.net(x)
        return linear_input_act(self.net[2], self.net[0].proj(x), "gelu_tanh")
...
def apply_rotary_emb(input_tensor, freqs_cis):
    ...
    if False:
        ...
        output = ck.apply_rope_split_half1(input_tensor, rotation_matrix)
    else:
        output = ck.apply_rope1(input_tensor, rotation_matrix)
...
def apply_rotary_emb_qk(q, k, freqs_cis):
    ...
    if split_pe:
        q, k = ck.apply_rope_split_half(q, k, rotation_matrix)
    else:
        q, k = ck.apply_rope(q, k, rotation_matrix)
...
class BasicTransformerBlock(nn.Module):
    ...
    def forward(self, x, context=None, attention_mask=None, timestep=None, pe=None, transformer_options={}, self_attention_mask=None, prompt_timestep=None):
        ...
        if False:
            norm_x = rms_norm(x) * (1 + scale_msa) + shift_msa
        else:
            norm_x = ck.rms_adaln(x, scale_msa, shift_msa)
...
def apply_cross_attention_adaln(
    ...
    if False:
        ...
        attn_input = ck.rms_adaln(x, q_scale, q_shift)
