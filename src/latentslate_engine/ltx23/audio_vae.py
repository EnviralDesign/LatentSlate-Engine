...
class Ltx23AudioMelDecoder:
    ...
    def __init__(self, checkpoint_path: str, device: str = "cuda") -> None:
        ...
        # Comfy denormalizes the float32 sampler latent before the audio VAE.
        # Its manual-cast convolutions consequently materialize float32 weights.
        self.decoder = autoencoder.decoder.to(device=device, dtype=torch.float32).eval()
        self._mean = autoencoder.latents_mean.to(
            device=device, dtype=torch.float32
        ).view(1, 8, 1, 16)
        self._std = autoencoder.latents_std.to(device=device, dtype=torch.float32).view(
            1, 8, 1, 16
        )
