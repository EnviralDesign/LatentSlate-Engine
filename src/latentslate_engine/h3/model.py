...
Adapted from ComfyUI 1a14b82e7339176357d627c41a262543a6a1356b
comfy/ldm/minimax/model.py (GPL-3.0), retaining native joint-AV math.
Graph wrappers, patch dispatch and global memory management are excluded.
...
from .contracts import validate_canvas
from .weights import Operations, swiglu_linear
...
class TimeEmbedder(nn.Module):
    def __init__(self, freq_dim, hidden, out, dtype=None, device=None, operations=None):
        ...
        self.proj_in = operations.Linear(
            freq_dim, hidden, bias=True, dtype=dtype, device=device
        )
        self.proj_out = operations.Linear(
            hidden, out, bias=True, dtype=dtype, device=device
        )
...
class Attention(nn.Module):
    def __init__(
        ...
        self.qkv_proj = operations.Linear(
            hidden, inner * 3, bias=False, dtype=dtype, device=device
        )
        ...
        if gate_compress:
            ...
            self.to_gate_compress = operations.Linear(
                hidden, inner, bias=False, dtype=dtype, device=device
            )
    ...
    def forward(self, x, rope_freqs=None):
        ...
        if rope_freqs is not None:
            ...
            qw = self.q_norm.weight.to(x.device)
            kw = self.k_norm.weight.to(x.device)
        ...
        out = attention(q, k, v).transpose(1, 2).reshape(s, -1)
        del q, k, v
...
class MLP(nn.Module):
    def __init__(self, hidden, ffn, dtype=None, device=None, operations=None):
        ...
        self.fc1 = operations.Linear(
            hidden, ffn * 2, bias=False, dtype=dtype, device=device
        )
    ...
    def forward(self, x):
        return swiglu_linear(self.fc2, self.fc1(x))
...
def _mod_scale_shift(h, shift, scale, segments):
    ...
    for a, b, row in segments:
        h[a:b].mul_(1.0 + _mod_row(scale, row, h.dtype)).add_(
...
class TokenRefiner(nn.Module):
    def __init__(
        ...
        self.final_norm = operations.RMSNorm(
            hidden, eps=final_eps, dtype=dtype, device=device
        )
...
class FinalLayer(nn.Module):
    def __init__(
        ...
        self.video_out = operations.Linear(
            hidden, video_dim, bias=True, dtype=torch.float32, device=device
        )
        self.audio_out = operations.Linear(
            hidden, audio_dim, bias=True, dtype=torch.float32, device=device
        )
    ...
    def forward(self, x, t_emb, video_seg, audio_seg):
        ...
        def mod(seg):
            a, b, row = seg
            return (
                self.norm(x[a:b]) * (1.0 + _mod_row(scale, row, scale.dtype))
                + _mod_row(shift, row, shift.dtype)
            ).to(torch.float32)
...
class MiniMaxH3Model(nn.Module):
    def __init__(
        ...
        video_patch_dim = (
            latents_dim * self.patch_size[0] * self.patch_size[1] * self.patch_size[2]
        )
        ...
        self.condition_proj = operations.Linear(
            text_dim, hidden_size, bias=True, dtype=dtype, device=device
        )
        ...
        self.rope.register_buffer(
            "inv_freq", torch.empty(rope_inv_freq_len, dtype=torch.float32)
        )
    ...
    def forward(self, x, timestep, context, minimax_payload=None):
        ...
        out = self._forward(x, timestep, context, minimax_payload=minimax_payload)

        if scale != 1.0:
            # d/d(sigma_v) of the carried variable
            out[1] = (1.0 - scale) * (audio_src * carry) + (
                1.0 + (scale - 1.0) * sigma_a
            ).to(out[1].dtype) * out[1]
        return out

    def _forward(self, x, timestep, context, minimax_payload=None):
        video_x, audio_x = x[0], x[1]
        orig_t, orig_h, orig_w = video_x.shape[2], video_x.shape[3], video_x.shape[4]
        validate_canvas(orig_w * 16, orig_h * 16)
        ...
        if self.use_adaln_curves:
            ...
            pos = (
                t_vals.clamp(0.0, 1.0) * (table.shape[0] - 1)
        ...
        video_out = unpatchify_video(
            v, latent_t, lat_h // 2, lat_w // 2, self.latents_dim, self.patch_size
        )
