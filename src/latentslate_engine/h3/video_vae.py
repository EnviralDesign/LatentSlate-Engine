"""H3 video codec adapted from ComfyUI 1a14b82e (GPL-3.0).

Preserves spatial tiling, temporal overlap and CPU output streaming.
"""
...
def _free_memory(device):
    ...
    return (
        free + stats["reserved_bytes.all.current"] - stats["active_bytes.all.current"]
    )
...
def _kitchen_ndhwc(x):
    ...
    return (
        ck is not None
        and hasattr(ck, "group_norm_silu_pad3d")
...
def _fused_norm_pad(x, norm, spatial_pad, front):
    ...
    if norm is None:
        return ck.group_norm_silu_pad3d(
            x, None, None, 1, 0.0, (*spatial_pad, front), silu=False
        )
    weight, bias = _to_input(norm.weight, x), _to_input(norm.bias, x)
    return ck.group_norm_silu_pad3d(
        x, weight, bias, norm.num_groups, norm.eps, (*spatial_pad, front), silu=True
    )


def _fp16_accum_conv(conv, x, weight, bias, residual):
    # kitchen fp16-accumulate conv, same opt-in as the fp16 GEMMs
    if not hasattr(ck, "fp16_conv3d") or not (
...
class CausalConv3d(ops.Conv3d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0):
        ...
        self.causal_padding = (
            (padding,) * 3 if isinstance(padding, int) else tuple(padding)
        )
    ...
    def forward(self, x, pre_norm=None, spatial_pad=None, residual=None):
        ...
        if (
            ...
            and not self.weight.is_contiguous(memory_format=torch.channels_last_3d)
        ):
        ...
        if (
            pre_norm is not None or front or any(spatial_pad)
        ):  # the 1x1 shortcut has nothing to fold
        ...
        if x.shape[2] == 1 and pad_t:
            ...
            out = F.conv3d(x, self.weight[:, :, -x.shape[2] :], self.bias, self.stride)
        elif ndhwc:
            weight_cl = self.weight.contiguous(memory_format=torch.channels_last_3d)
            out = _fp16_accum_conv(self, x, weight_cl, self.bias, residual)
            if out is not None:
                return out
            out = F.conv3d(x, weight_cl, self.bias, self.stride)
        else:
            out = super().forward(x)
...
def group_norm_3d(num_channels):
    return TemporalIsolatedGroupNorm(
...
class FeedForward(nn.Module):
    ...
    def forward(self, x, pre_norm, residual, residual_scale):
        ...
        gate, up = self.w1(pre_norm(x)).chunk(2, dim=-1)
...
class Attention(nn.Module):
    ...
    def forward(self, x, rotary_pos_emb, pre_norm, residual, residual_scale):
        ...
        if rotary_pos_emb is not None:
            ...
            rms_rope = (
                ck.rms_rope_split_half
                if torch.is_grad_enabled()
                else ck.rms_rope_split_half_
            )
            query, key = rms_rope(
                query,
                key,
...
class TransformerBlock(nn.Module):
    def __init__(self, heads, dim_head, bias=True, eps=1e-5, operations=ops):
        ...
        self.attn = Attention(
            heads=heads, dim_head=dim_head, bias=bias, eps=eps, operations=operations
        )
    ...
    def forward(self, x, rotary_pos_emb=None):
        x = self.attn(
            ...
            pre_norm=self.norm1,
            residual=x,
            residual_scale=self._residual_scale(self.scale1, x),
        )
        return self.ff(
            x,
            pre_norm=self.norm2,
            residual=x,
            residual_scale=self._residual_scale(self.scale2, x),
        )
...
class ViT3DDecoder(nn.Module):
    ...
    def forward(self, x):
        ...
        h = torch.cat(
            [
                h,
                _to_input(self.register_tokens, h).expand(B, -1, -1),
                torch.zeros_like(h[:, 0:1, :]),
            ],
            dim=1,
        )

        img_ids = create_token_ids(
...
class MiniMaxH3VideoVAE(nn.Module):
    ...
    def _normalize_pixels(self, x):
        return (
            x.add(1.0).mul_(0.5).sub_(self.pixel_mean.to(x)).div_(self.pixel_std.to(x))
        )
    ...
    def _finalize_pixels(self, part):
        ...
        return part.add_(
            self.pixel_mean.to(device=part.device, dtype=torch.float32)
        ).clamp_(0.0, 1.0)
    ...
    def _decode_tile_row(self, z_row, x_idx, x_len):
        ...
        free = _free_memory(z_row.device)
        batch = int(max(1, min(4, free // (128 * 2**20 * z_row.shape[0]))))
        slices = [
            z_row[..., j_pos // self.vae_ratio : (j_pos + j_len) // self.vae_ratio]
            for j_pos, j_len in zip(x_idx, x_len)
        ]
    ...
    def decode_temporal(self, z, output_buffer=None):
        ...
        if output_buffer is None:
            ...
            output_buffer = torch.empty(
                self.decode_output_shape(z.shape), dtype=torch.float32, device="cpu"
            )
