"""H3 stereo audio codec, adapted from ComfyUI 1a14b82e (GPL-3.0).

Retains the DAC/BigVGAN lineage and license notices below.
"""
...
class SnakeBeta(nn.Module):
    ...
    def forward(self, x):
        alpha = torch.exp(_to_input(self.alpha, x)).view(1, -1, 1)
        beta = torch.exp(_to_input(self.beta, x)).view(1, -1, 1)
...
class UpSample1d(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.conv_transpose1d(
            x, _to_input(self.filter.expand(C, -1, -1), x), stride=self.stride, groups=C
        ).mul_(self.ratio)
...
class CausalAttention(nn.Module):
    ...
    def forward(self, x):
        B, N, _ = x.shape
        weight = _to_input(self.qkv.weight, x)
        qkv = F.linear(
            x,
            weight=weight,
            bias=_to_input(torch.cat((self.q_bias, self.zero_k_bias, self.v_bias)), x),
        )
        q, k, v = (
...
class AMPBlock1(nn.Module):
    def __init__(self, channels, kernel_size=3, dilation=(1, 3, 5)):
        ...
        self.activations = nn.ModuleList(
            [
                Activation1d(activation=SnakeBeta(channels))
