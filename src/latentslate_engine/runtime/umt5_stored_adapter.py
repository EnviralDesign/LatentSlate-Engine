...
def plan_comfy_umt5_encoder(
    ...
    if probe.quantization_contract not in _SUPPORTED:
        raise ValueError(f"UMT5 stored adapter: unsupported artifact contract {probe.quantization_contract!r}")
    ...
    precision_errors = list(_validate_header_contract(probe.quantization_contract, header, source_to_targets, dense_dtypes))
...
def materialize_umt5_encoder(
    ...
    if {target for targets in plan.source_to_targets.values() for target in targets} != expected_targets:
        raise ValueError("UMT5 materializer: plan targets do not exactly match this encoder skeleton")
    ...
    try:
        with safe_open(str(plan.identity.path), framework="pt", device="cpu") as handle:
            if not revalidate_artifact(plan.identity):
                raise ValueError("UMT5 materializer: artifact identity changed before materialization")
            ...
            if any(handle.get_slice(key).get_dtype() != dtype for key, dtype in plan.dense_source_dtypes.items()):
            ...
            for source, layer in quant_layers.items():
                ...
                if attribute != "weight" or not isinstance(parent, nn.Linear):
                    raise TypeError("UMT5 materializer: stored quant is permitted only for nn.Linear weights")
                ...
                if input_scale in keys:
                    ...
                    if value.dtype != torch.float32 or tuple(value.shape) != tuple(scale.shape) or not torch.equal(value, scale):
                        raise ValueError("UMT5 materializer: ConvRot input_scale is not the proven stored scale alias")
            ...
            if consumed_auxiliary != expected_aux:
                raise ValueError("UMT5 materializer: unconsumed or unexpected quantization auxiliaries")
...
def _plan_quant_roles(
    ...
    for source, targets in mapping.items():
        ...
        permitted = {stem + ".scale_weight"} if contract == "comfy_legacy/scaled_fp8_e4m3fn" else {
            stem + ".weight_scale", stem + ".input_scale", stem + ".comfy_quant"
        }
        present = {key for key in keys if key.startswith(stem + ".") and key.endswith(_AUX_SUFFIXES)}
        if not present:
            continue
        ...
        if contract == "comfy_quant/int8_tensorwise_convrot":
            ...
            if (
                ...
                errors.append("UMT5 ConvRot marker or scale header does not match the stored per-row contract")
                continue
        else:
            scale = header[stem + ".scale_weight"]
            if _entry_dtype(scale, stem + ".scale_weight") != "F32" or _entry_shape(scale, stem + ".scale_weight") != ():
                errors.append("UMT5 legacy FP8 scale header does not match the scalar F32 contract")
                continue
        quant_sources.append(source)
    if contract == "comfy_quant/int8_tensorwise_convrot":
        errors.extend(_validate_convrot_global_metadata(header.get("__metadata__"), quant_sources, header))
...
def _describe_quant_layers(handle, plan: UMT5StoredAdapterPlan) -> dict[str, StoredQuantizedLayer]:
    ...
    return describe_stored_layers_from_handle(handle, identity=plan.identity, keys=keys, contract=plan.artifact_contract)
    ...
    for child in module.modules():
        ...
        for name, buffer in tuple(child._buffers.items()):
            if buffer is not None:
                child._buffers[name] = torch.empty(tuple(buffer.shape), dtype=buffer.dtype, device="meta")
...
def _validate_header_contract(
    ...
    if dense_mismatches:
        errors.append(f"UMT5 dense precision contract requires F16: {dense_mismatches} mismatch(es)")
    ...
    for source in set(mapping) - set(dense_dtypes):
        if _entry_dtype(header[source], source) != expected_q_dtype:
            ...
            break
        stem = source.removesuffix(".weight")
        required = {stem + ".scale_weight"} if contract == "comfy_legacy/scaled_fp8_e4m3fn" else {
            stem + ".weight_scale", stem + ".input_scale", stem + ".comfy_quant"
        }
    ...
    if not isinstance(spiece, dict) or spiece.get("dtype") != "U8":
        errors.append("UMT5 artifact requires an embedded U8 spiece_model")
    sentinel = header.get("scaled_fp8")
    if contract == "comfy_legacy/scaled_fp8_e4m3fn":
        if not isinstance(sentinel, dict) or sentinel.get("dtype") != "F8_E4M3" or sentinel.get("shape") != [0]:
...
def _config_fingerprint(config: Mapping[str, Any]) -> str:
    raw = json.dumps(dict(sorted(config.items())), sort_keys=True, separators=(",", ":"), allow_nan=False).encode("utf-8")
