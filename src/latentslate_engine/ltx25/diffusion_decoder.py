...
Narrow GPL-3.0 adaptation of ComfyUI 1a14b82e
...
from latentslate_engine.ltx23.model import get_timestep_embedding
...
def patchify(x, patch_size_hw, patch_size_t=1):
    if patch_size_hw == 1 and patch_size_t == 1:
        return x
    b, c, t, h, w = x.shape
    p, q = patch_size_t, patch_size_hw
    return (
        x.reshape(b, c, t // p, p, h // q, q, w // q, q)
        .permute(0, 1, 3, 7, 5, 2, 4, 6)
        .reshape(b, c * p * q * q, t // p, h // q, w // q)
    )
...
def unpatchify(x, patch_size_hw, patch_size_t=1):
    if patch_size_hw == 1 and patch_size_t == 1:
        return x
    b, channels, t, h, w = x.shape
    p, q = patch_size_t, patch_size_hw
    c = channels // (p * q * q)
    return (
        x.reshape(b, c, p, q, q, t, h, w)
        .permute(0, 1, 5, 2, 6, 4, 7, 3)
        .reshape(b, c, t * p, h * q, w * q)
    )
...
def rope_inv_freqs(dim, base=10000.0, device=None):
    out_device = device
    if torch.device(device or "cpu").type not in {"cpu", "cuda"}:
    ...
    return (
        1.0
        / torch.pow(
            torch.tensor(float(base), dtype=torch.float64, device=device), exponents
        )
    ).to(dtype=torch.float32, device=out_device)
...
def _rope_matrices_slice(tables, t0, t1, h, w):
    ...
    for (c, s), sl in zip(tables, (slice(t0, t1), slice(None), slice(None))):
        c, s = c[sl], s[sl]
        parts.append(
            torch.stack([c, -s, s, c], dim=-1).reshape(
                c.shape[0], 1, 1, c.shape[1], 2, 2
            )
        )
    ts = t1 - t0
    freqs = torch.cat(
...
class NeighborhoodAttention3D(nn.Module):
    ...
    def forward(self, x, pre=None, add_to=None):
        ...
        inv_freqs = tuple(
            rope_inv_freqs(d, self.rope_base, device=x.device) for d in self.rope_split
        )
        ...
        q_weight = (self.q_norm.weight.detach() * self.scale).to(
            x.dtype
        )  # scale commutes with the rotation
...
class AdaLNZero(nn.Module):
    ...
    def forward(self, t_emb):
        ...
        return tuple(
            c[:, None, None, None, :] for c in h.chunk(self.NUM_CHUNKS, dim=-1)
        )
...
class DiffusionNABlock(nn.Module):
    ...
    def forward(self, x, latent_context, modulation):
        scale_msa, shift_msa, _, scale_mlp, shift_mlp, _, _ = [
            ...
            for i in range(AdaLNZero.NUM_CHUNKS)
        ]
        chunk = max(1, MLP_TOKEN_CHUNK // max(x.shape[2] * x.shape[3], 1))
        for t0 in range(0, x.shape[1], chunk):
            x[:, t0 : t0 + chunk] += self.context_proj(
                latent_context[:, t0 : t0 + chunk]
            )
        x = self.attn(
            x, pre=lambda s: modulate(self.norm1(s), scale_msa, shift_msa), add_to=x
        )
        return self.mlp(
            x, pre=lambda s: modulate(self.norm2(s), scale_mlp, shift_mlp), add_to=x
        )
...
class LinearPixelShuffleUpsample(nn.Module):
    ...
    def forward(self, x, drop_leading_frame=True):
        ...
        out = torch.empty(
            (batch, t * p1, h * p2, w * p3, self.out_channels),
            dtype=x.dtype,
            device=x.device,
        )
        chunk = max(1, MLP_TOKEN_CHUNK // max(h * w, 1))
        for t0 in range(0, t, chunk):
            t1 = min(t0 + chunk, t)
            projected = self.proj(x[:, t0:t1])
            out[:, t0 * p1 : t1 * p1] = (
                projected.reshape(batch, t1 - t0, h, w, self.out_channels, p1, p2, p3)
                .permute(0, 1, 5, 2, 6, 3, 7, 4)
                .reshape(batch, (t1 - t0) * p1, h * p2, w * p3, self.out_channels)
            )
            del projected
...
class NADiffusionDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer(
            ...
            torch.linspace(
                1.0, 1.0 / default_num_inference_steps, default_num_inference_steps
            ),
            persistent=False,
        )
        ...
        for stage_i in range(len(stage_channels) - 1):
            ...
            self.det_stages.append(
                nn.ModuleList(
                    [
                        NABlock(c, stage_kernels[stage_i], head_dim=head_dim)
            ...
            self.upsamples.append(
                LinearPixelShuffleUpsample(
                    c, stride, out_channels_reduction_factor=reduction
                )
            )
        ...
        self.diff_blocks = nn.ModuleList(
            [
                DiffusionNABlock(
                    c5, stage5_kernel, context_channels=c5, head_dim=head_dim
                )
    ...
    def forward(self, z, generator=None, drop_leading_frame=True, pad_trailing=True):
        context = self.forward_pre_diffusion(
            z, drop_leading_frame=drop_leading_frame, pad_trailing=pad_trailing
        )
        ...
        x_t = torch.randn(
            pixel_shape, dtype=z.dtype, device=z.device, generator=generator
        )
        ...
        for i in range(num_steps):
            ...
            t_next = (
                timesteps[i + 1]
                if i + 1 < num_steps
                else torch.zeros_like(timesteps[i])
            )
