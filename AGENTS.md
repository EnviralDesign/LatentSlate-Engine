...
- Localize before porting: map executed Comfy stages to Engine equivalents and compare cold/warm time, RAM and VRAM, including incoming state and allocation lifetimes. Before changing Engine, identify the exercised Comfy implementation, the concrete behavioral disagreement, and evidence connecting it to the measured gap. Adapt the smallest coherent implementation with its required ownership/dependency semantics; do not substitute speculative Engine-only optimizations for unresolved reference differences. Follow the reconciliation procedure in `docs/COMFY_REFERENCE.md` and validate the isolated boundary plus a representative pressure case before full certification.
