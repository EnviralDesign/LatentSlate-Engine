...
def test_all_builtins_publish_execute_and_preserve_builtins(tmp_path):
    ...
    with TestClient(create_app(home=tmp_path, token="", executor=runtime)) as client:
        ...
        assert catalog[:13] == builtins
        assert len(set(ids)) == 13
        assert len(catalog) == 26
        assert len(runtime.recipes) == 13
    with TestClient(
        create_app(home=tmp_path, token="", executor=RecipeRuntime())
    ) as client:
        assert {
            item["id"] for item in client.get("/v1/catalog").json()["tools"][13:]
        } == set(ids)
...
def test_restart_reconciles_changed_projection_without_recipe_edits(
    ...
    for _ in range(2):
        with TestClient(
            ...
            published = client.get("/v1/catalog").json()["tools"][13:]
...
def test_hidden_revision_freshness_stable_identity_and_cross_host_import(tmp_path):
    ...
    with TestClient(
        ...
        assert len(target.get("/v1/catalog").json()["tools"]) == 13
...
def test_unavailable_publication_and_builtin_identity_collision(tmp_path, monkeypatch):
    ...
    with TestClient(
        ...
        assert len(client.get("/v1/catalog").json()["tools"]) == 13
...
def test_remote_fresh_hosts_publish_and_execute_through_normal_jobs(
    ...
    for host in ("source", "fresh-import"):
        ...
        with TestClient(app) as client:
            ...
            assert client.get("/v1/catalog").json()["tools"][:13] == frozen
