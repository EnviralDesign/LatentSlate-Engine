...
def test_persistent_payload_is_private_and_rejects_preexisting_output(tmp_path: Path) -> None:
    paths = managed_module._worker_paths(tmp_path / "visible.mp4")
    assert paths.request.parent != tmp_path
    assert paths.request.parent.name.startswith("latentslate-wan14-")
    assert {
        paths.request.name,
        paths.result.name,
        paths.progress.name,
        paths.heartbeat.name,
        paths.start_gate.name,
        paths.command.name,
        paths.cancel.name,
    } == {
        "request.json",
...
def test_worker_result_rejects_unexpected_output_path(tmp_path: Path):
    ...
    atomic_write_json(result, value)
    with pytest.raises(RuntimeError, match="unexpected output"):
        managed_module._read_persistent_result(
            result,
            expected_output=expected,
            expected_binding="request",
            secret=secret,
        )
...
def test_supervisor_rejects_preexisting_private_ipc_paths(tmp_path: Path):
    ...
    with pytest.raises(RuntimeError, match="already exist: progress"):
        managed_module._require_fresh(paths, initial=True)


def test_unload_delegates_tree_proof_and_cleanup_to_shared_supervisor():
    managed = ManagedNativeWanI2VRuntime(
    ...
    class _Supervisor:
        session = SimpleNamespace(process=SimpleNamespace(pid=5, poll=lambda: 0))
    ...
    managed._session = managed_module._WanWorkerSession(supervisor, "cuda", "binding", b"x" * 32)
    managed._active_supervisor = supervisor
    managed.unload()
