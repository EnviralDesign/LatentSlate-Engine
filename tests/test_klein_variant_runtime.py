...
def _optimizations(**updates):
    values = {
        "attention": "inherit", "offload": "inherit", "quantization": "inherit",
        "compile": False, "compile_mode": "default", "compile_fullgraph": False,
        "compile_dynamic": False, "vae_tiling": "inherit", "vae_slicing": "inherit",
        "cache": "inherit", "group_offload_blocks": None,
        "group_offload_use_stream": False, "group_offload_record_stream": False,
        "low_cpu_mem_usage": True, "keep_pipeline_loaded": True,
    }
    values.update(updates)
    return values


def test_klein_advertises_only_currently_proven_native_artifacts():
...
def test_klein_rejects_unsafe_nonconversion_mode_combinations():
    ...
    lora_compile = ExecutionRequest(
        family="klein4b", loras=True, lora_formats=frozenset({"safetensors"}),
    ...
    group_stream_tiling = ExecutionRequest(
        ...
        optimizations=_optimizations(
            offload="group_block", group_offload_blocks=2,
            group_offload_use_stream=True, vae_tiling="on",
        ),
    )
    assert any("LoRA switching is not supported on a compiled" in reason for reason in tool.validate_execution_request(lora_compile))
...
def test_model_override_and_dynamic_lora_share_pipeline_fingerprint(tmp_path: Path):
    ...
    common = {
        "family": "klein4b", "model_resource_id": "model:klein4b:custom-model",
        "model_path": model, "model_format": "diffusers", "model_precision": "bf16",
        "model_quantization": "native",
    }
    first = resolve_klein_runtime_plan(
        settings, "klein4b", ExecutionPlan(variant_key="first", **common, optimizations=_optimizations(attention="flash4_hub", quantization="bf16", cache="prompt")),
    )
    second = resolve_klein_runtime_plan(
        settings, "klein4b", ExecutionPlan(
            variant_key="second", **common,
            loras=(LoraExecution("style", "lora:klein4b:style", lora_path, 0.6),),
            optimizations=_optimizations(attention="flash4_hub", quantization="bf16", cache="media", keep_pipeline_loaded=False),
        ),
    )
...
def test_klein_runtime_manager_key_reuses_pipeline_across_lora_changes(tmp_path, monkeypatch):
    ...
    first_plan = resolve_klein_runtime_plan(settings, "klein4b", ExecutionPlan(variant_key="first", **common, optimizations=_optimizations(quantization="bf16")))
    second_plan = resolve_klein_runtime_plan(settings, "klein4b", ExecutionPlan(variant_key="second", **common, optimizations=_optimizations(quantization="bf16", cache="none")))
    accelerated_plan = resolve_klein_runtime_plan(settings, "klein4b", ExecutionPlan(variant_key="accelerated", **common, optimizations=_optimizations(quantization="bf16", attention="flash4_hub")))
    created = []
