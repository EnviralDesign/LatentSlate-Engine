...
| [Wan 2.2 14B](./WAN22_14B.md) | Matching official dense BF16 T2V or I2V expert pair | None; active shift-5 Comfy FP8 I2V and T2V baselines are **Fallback** | LightX2V I2V is accepted separately as **Experimental**; Winnougan INT8 ConvRot resources are cataloged but not runnable | Exact I2V and T2V baseline success, denoise cancellation, fresh recovery, native LoRA dispatch, disposable-worker teardown, and ConvRot catalog metadata accepted; exact ConvRot header planning/native dispatch remain unproven | Complete corpus, source-invalidation, peer-switch, and BF16-reference quality evidence; qualify the official T2V LightX path, pin a clean ConvRot operation, and prove its exact-header planner/dispatch path before recipe exposure |
