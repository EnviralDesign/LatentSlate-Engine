...
Remote acquisition lives above `Recipe`: `artifact_sources.py` validates
portable identities and owns Hub metadata/streaming; `civitai_source.py` owns
exact Civitai version/file inspection and acquisition. `artifact_materialization.py`
...
continue receiving ordinary local artifacts. A two-entry source map dispatches
metadata/download operations; both sources use the same tasks and digest-only
cache, including cross-source reuse. There is no provider registry or model manager.
